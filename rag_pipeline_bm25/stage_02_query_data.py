...
def query_rag(query_text: str, bm25_db_dir, bm25_db_path, bm25_metadata_path, at_k, at_r, logger):
    try:
        try:
            logger.info("[Stage 02, Part 01] Querying TF-IDF DB.....")
            
            # Load the existing TF-IDF db (prep the db)
            if bm25_db_dir.exists():
                with open(bm25_db_path, "rb") as f:
                    bm25, texts, tokenized_texts  = pickle.load(f)

                with open(bm25_metadata_path, "r", encoding="utf-8") as f:
                    metadata = json.load(f)
                logger.info(f"[Stage 02, Part 01.1] Loading existing TF-IDF DB from path: {bm25_db_dir}.....")
                
                if not isinstance(metadata, list):
                    logger.error("[Stage 02, Part 01.1] Metadata JSON is not a list.")
                    return []
            else:
                logger.error(f"[Stage 02, Part 01.1] TF-IDF DB not found at path: {bm25_db_dir}. Please populate the DB first.")
                return []
            ...
            logger.info(f"[Stage 02, Part 01.2] Searching the db with text using BM25 Scoring: {query_text}.....")
            def bm25_similarity_search_with_score(query_text, bm25, documents, at_k):
                # Tokenize query
                ...
                # Get top-k results
                top_k_idx = scores.argsort()[-at_k:][::-1]
                return [(documents[i], scores[i]) for i in top_k_idx]

            results = bm25_similarity_search_with_score(query_text, bm25, documents, at_k)
        ...
        logger.error(f"Error in querying BM25 DB & grading: {e}")
        logger.debug(traceback.format_exc())
        return {
            'llms_response': '',
            'context': '',
...
def run_query_rag(query: str, bm25_db_dir=BM25_DB_DIR, bm25_db_path=BM25_DB_PATH, bm25_metadata_path=BM25_META_PATH, at_k=K, at_r=R) -> str:
    try:
        logger = setup_logger("query_rag_logger", LOG_FILE)
        logger.info(" ")
        logger.info("++++++++Starting Querying, Retrieval Grading, & Generation stage....")
        
        results = query_rag(query, bm25_db_dir, bm25_db_path, bm25_metadata_path, at_k, at_r, logger)
        if query is None:
