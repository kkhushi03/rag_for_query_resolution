...
from rag_pipeline_bm25.stage_02_query_data import query_rag
...
def evaluate_single_query(question: str, ground_truth: str, relevant_doc_ids: List[str], bm25_db_dir: Path, bm25_db_path: Path, bm25_metadata_path: Path, at_k: int, at_r: int, max_n: int, logger) -> Dict[str, Any]:
    start_time = time.time()

    try:
        logger.info(f"[Stage 03, Part 02] Processing question: {question}")
        rag_result = query_rag(query_text=question, bm25_db_dir=bm25_db_dir, bm25_db_path=bm25_db_path, bm25_metadata_path=bm25_metadata_path, at_k=at_k, at_r=at_r, logger=logger)
...
def run_evaluation(eval_dir=EVALUATION_DATA_PATHS, result_dir=RESULTS_CSV_PATH, bm25_db_dir=BM25_DB_DIR, bm25_db_path=BM25_DB_PATH, bm25_metadata_path=BM25_META_PATH, at_k=K, at_r=R, max_n=MAX_N, embedding_model=EMBEDDING_MODEL, local_llm=LOCAL_LLM):
    ...
    for i, item in enumerate(eval_data):
        ...
        result = evaluate_single_query(
            ...
            bm25_metadata_path=bm25_metadata_path,
            at_k=at_k,
            at_r=at_r,
            max_n=max_n,
            logger=logger
