...
from rag_pipeline_hybrid.stage_02_query_data import query_rag
...
LOG_FILE = os.path.join(LOG_DIR, "hybrid_stage_03_evaluate_rag.log")
...
def evaluate_single_query(question: str, ground_truth: str, relevant_doc_ids: List[str], faiss_db_dir, bm25_db_path, faiss_weight, bm25_weight, at_k: int, at_r: int, max_n: int, logger) -> Dict[str, Any]:
    start_time = time.time()

    try:
        logger.info(f"[Stage 03, Part 02] Processing question: {question}")
        rag_result = query_rag(query_text=question, faiss_db_dir=faiss_db_dir, bm25_db_path=bm25_db_path, faiss_weight=faiss_weight, bm25_weight=bm25_weight, at_k=at_k, at_r=at_r, logger=logger)
...
def run_evaluation(eval_dir=EVALUATION_DATA_PATHS, result_dir=RESULTS_CSV_PATH, faiss_db_dir=HYBRID_FAISS_DB_DIR, bm25_db_path=HYBRID_BM25_DB_PATH, faiss_weight=FAISS_WEIGHT, bm25_weight=BM25_WEIGHT, at_k=HYBRID_TOP_K, at_r=R, max_n=MAX_N, embedding_model=EMBEDDING_MODEL, local_llm=LOCAL_LLM):
